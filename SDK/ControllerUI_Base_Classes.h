...
namespace CG
{
	...
	public:
		...
		int32_t                                                    CurrentToggleValue;                                      // 0x0370(0x0004) Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, IsPlainOldData, NoDestructor, HasGetValueTypeHash
		unsigned char                                              UnknownData_JIBC[0x4];                                   // 0x0374(0x0004) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
		TArray<class FText>                                        ToggleValueTexts;                                        // 0x0378(0x0010) Edit, BlueprintVisible, DisableEditOnInstance
	...
	public:
		...
		void GetCurrentToggleTextType(int32_t* CurrentType);
		void GetToggleValueTexts(TArray<class FText>* FullTextValues);
		...
		void Set_Toggle_Value(int32_t NewValue);
		class UWidget* OnToggleNext(EUINavigation Navigation);
		class UWidget* OnTogglePrevious(EUINavigation Navigation);
		void GetToggleButton(class UButton** ToggleButton);
		void GetCorrectedButtonValueForC(EControllerButtonIDDefinitions InValue, int32_t* C_Value);
		void SetupControllerText();
		void GetMappingObjects(TArray<class USimpleControllerMappingText_C*>* ReturnArray1);
		class USimpleControllerMappingText_C* Create_Mapping_Text(const struct FControllerButtonLayout& ControllerButtonLayout, int32_t ArrayIndex);
		void Construct();
		void Tick(const struct FGeometry& MyGeometry, float InDeltaTime);
		void OnSynchronizeProperties();
		void BndEvt__ToggleButton_K2Node_ComponentBoundEvent_0_OnButtonClickedEvent__DelegateSignature();
		void PreConstruct(bool IsDesignTime);
		void ExecuteUbergraph_ControllerUI_Base(int32_t EntryPoint);
